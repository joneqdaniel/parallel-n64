...
- The repo-default `paper-mario-title-screen.sh`, `paper-mario-file-select.sh`, and `paper-mario-kmr-03-entry-5.sh` authority scenarios now use that same preference order when no cache override is provided and fail closed if none of those `PHRB` artifacts exist, so the normal `on` authority lane no longer silently falls back to the legacy `.hts` cache.
