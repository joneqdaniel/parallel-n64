...
- Libretro now exposes the same runtime policy as the `parallel-n64-parallel-rdp-hirestex-source-mode` core option. The frontend default is `auto`, while scenario env overrides still take precedence when a lane needs to force `phrb-only`.
- The low-level provider default now matches that same runtime default: `ReplacementProvider::load_cache_dir(path)` uses `auto` unless a caller passes `All`, `PHRBOnly`, or `LegacyOnly` explicitly, so mixed-source tests and tooling must opt into `All` when they intentionally want both formats loaded together.
...
- When the enriched full-cache package is the active conformance input, `emu.conformance.paper_mario_full_cache_phrb_authorities` now enforces the locked authority screenshot hashes as well as `native_sampled_entry_count > 0`, `descriptor_path_class=sampled-only`, `descriptor_path_counts.generic == 0`, and `descriptor_path_counts.compat == 0` for every authority fixture. When that same wrapper is pointed explicitly at the zero-config artifact through `EMU_RUNTIME_PM64_FULL_CACHE_PHRB`, it instead enforces the locked zero-config screenshot hashes plus `entry_class=compat-only` and `descriptor_path_class=compat-only` on every authority fixture.
- `emu.conformance.paper_mario_full_cache_phrb_authorities_refresh` goes one step further: it regenerates the enriched package from the legacy `.hts` cache plus the authority-summary-root context, then re-runs the full authority validation and asserts the current converter/runtime counts (`8883` package records, `28` runtime-ready native sampled records, `15` deterministic bindings, `13` unresolved overlay families, `503` live native sampled entries, `context_bundle_class=context-enriched`, `package_manifest_runtime_ready_record_class=mixed-native-and-compat`, and the current locked sampled-only descriptor-path counts per fixture). The support-level full-cache contracts now also require the accompanying `hts2phrb-runtime-overlay-review.*` artifact and the top-level `runtime_overlay_blockers` field, and assert the current overlay residue class directly: still `proxy-transport-selection-required`, but now also `8` `pixel-divergent-single-dim` cases, `5` `pixel-divergent-multi-dim` cases, zero hash-decode failures, and the current exact overlap profile (`identical_alpha_hash_case_count_counts = 0:9, 1:4`).
