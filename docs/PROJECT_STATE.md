...
- Local converter breadth currently covers:
  - the current Paper Mario legacy cache (Rice CRC, old format)
  - the repo-local pre-v401 Paper Mario legacy cache (Rice CRC, old format)
  - SM64 Reloaded v2.6.0 HD (Rice CRC, GlideN64 format, 2530 entries)
  - OoT Reloaded v11.0.0 HD (Rice CRC, GlideN64 format, 43267 entries)
- SM64 and OoT packs parse and convert cleanly but produce zero runtime hits (confirmed by 30-second boot test on SM64: 55,227 lookups, 0 hits).
- Root cause: our runtime computes Rice CRC from **TMEM** (`cpu_tmem`, 4KB, interleaved N64 texture memory). GlideN64 packs compute Rice CRC from **RDRAM** (linear source data). Same algorithm, different input data. The Paper Mario HTS was generated by a Glide64mk2-era tool that also CRCs TMEM (format version `0x40a20000`).
- Cross-game runtime validation requires either Glide64mk2-era packs (same TMEM CRC origin) or adding an RDRAM-based CRC lookup path to the runtime. The RDRAM address is already available at lookup time (`addr` in provenance telemetry).
