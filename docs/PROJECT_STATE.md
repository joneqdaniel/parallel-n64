...
- The first concrete plan slices are already in:
  ...
  - `--reuse-existing` now also invalidates on runtime-class drift, so a stale manifest cannot be reused just because the total runtime-ready count still matches while the native-vs-compat split no longer does
