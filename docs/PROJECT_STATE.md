...
- The first concrete plan slices are already in:
  ...
  - current local outcome on that slice: `10` requested families, `10` canonical package records, `0` runtime-ready package records, `0` deterministic bindings, `0` unresolved transport cases, `runtime_overlay_built=true`, `runtime_overlay_artifacts_emitted=true`, `conversion_outcome=canonical-package-only`, and two blocker classes (`missing-active-pool-families=10`, `canonical-only-families=10`)
  ...
  - practical implication: converter operational telemetry is now real and the canonical-package path is no longer blocked on runtime bindings; bundle-driven runs still build the overlay stage automatically, but the current timeout slice no longer pretends any runtime-ready package records exist
  ...
  - current local full-cache outcome: `8992` requested families, `8992` canonical package records, `8613` runtime-ready package records, `379` runtime-deferred package records, `0` deterministic bindings, `0` unresolved transport cases, `runtime_overlay_built=false`, `runtime_overlay_reason=no-runtime-context`, `runtime_overlay_artifacts_emitted=false`, `conversion_outcome=partial-runtime-package`, about `3.93 s` total runtime, and a single remaining blocker class (`canonical-only-families=379`)
  ...
  - the default zero-config artifact at [`artifacts/hts2phrb/paper-mario-hirestextures-9fa7bc07-all-families/hts2phrb-report.json`](/home/auro/code/parallel-n64/artifacts/hts2phrb/paper-mario-hirestextures-9fa7bc07-all-families/hts2phrb-report.json) has been regenerated and now matches the current full-cache state (`partial-runtime-package`, `8613` runtime-ready records); the refreshed report also makes the class split explicit: `context_bundle_class=zero-context`, `package_manifest_runtime_ready_record_class=compat-only`, `0` runtime-ready native-sampled records, `8613` runtime-ready compat records, `package_manifest_runtime_deferred_record_class=compat-only`, and `379` deferred compat records. Earlier repo-local `canonical-package-only` all-families outputs are stale and should not be reused
  ...
  - runtime source policy now also exposes an explicit opt-in `auto` mode, so mixed cache directories can prefer `.phrb` automatically without changing the current unset-env default of `all`
...
- The current next execution slice is:
  ...
  - fresh authority reruns also show that the active title-screen, file-select, and `kmr_03 ENTRY_5` fixtures are still intentionally on the legacy default pack path rather than the selected-package path
    - current authority outcome: `source_mode=legacy-only`, `native_sampled=0`, `sources(phrb=0, hts=15168, htc=0)` on all three active `on` authorities
    - practical implication: the fixture gate can now verify provider source mode, but native-`PHRB` minimums should stay deferred until the authority lane itself moves to `PHRB` inputs or default-path promotion begins
