...
- A deterministic cold-boot trace now shows the vanilla callback path clearly: `startup -> logos -> intro`; there is no passive drift into a real `title_screen` callback state
- Deterministic early-boot controller pulses still do not alter that callback path, and direct `gGameStatus` button snapshots remain all-zero during those boot probes, so the current agent input override is not yet authoritative for early startup/logos automation even though it works reliably from later savestate-backed fixtures
