...
- The first concrete plan slices are already in:
  ...
  - there is now one tracked review-only overlay reduction profile at [`tools/hires_runtime_overlay_review_profile.json`](/home/auro/code/parallel-n64/tools/hires_runtime_overlay_review_profile.json) backed by [`tools/hires_runtime_overlay_review_transport_policy.json`](/home/auro/code/parallel-n64/tools/hires_runtime_overlay_review_transport_policy.json). The current persistent proof at [`artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json) cuts the enriched full-cache overlay residue from `13` unresolved to `9`, raises deterministic bindings from `15` to `19`, and now makes the improved shape explicit as `8` direct overlay cases plus the same `1` import-linked case. It also clears the grouped candidate-set review set entirely (`runtime_overlay_candidate_set_review_group_count=0`), and still passes the strict [`emu.conformance.paper_mario_full_cache_phrb_authorities`](/home/auro/code/parallel-n64/tests/emulator_behavior/support/emu_conformance_paper_mario_full_cache_phrb_authorities.sh) wrapper. It remains explicitly review-only.
