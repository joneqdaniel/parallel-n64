...
- The current active merged selected package is [`20260329-selected-plus-title-v18-edge-fallback/package.phrb`](/home/auro/code/parallel-n64/artifacts/hires-pack-review/20260329-selected-plus-title-v18-edge-fallback/package.phrb).
  - Build inputs:
  ...
  - Runtime proofs:
    - title: [`20260329-v18-edge-fallback-probe`](/home/auro/code/parallel-n64/artifacts/paper-mario-title-screen/on/20260329-v18-edge-fallback-probe)
    ...
    - file select: [`20260329-v18-edge-fallback-probe`](/home/auro/code/parallel-n64/artifacts/paper-mario-file-select/on/20260329-v18-edge-fallback-probe)
...
- Practical implication: the current active merged package is now self-contained at surface-package compile time too. It reproduces the same strict-scene runtime outputs even when the surface-package provenance `review_path` values are deliberately broken, so the merged title slice no longer depends on external review JSON during build or runtime.
  ...
  - new native-path probe evidence: [`20260329-v18-edge-fallback-probe`](/home/auro/code/parallel-n64/artifacts/paper-mario-title-screen/on/20260329-v18-edge-fallback-probe) plus the probe-only control [`20260329-selected-plus-title-v13-surface-v2-probe-runtime`](/home/auro/code/parallel-n64/artifacts/paper-mario-title-screen/on/20260329-selected-plus-title-v13-surface-v2-probe-runtime) show that reintroducing `28916d63` is runtime-neutral on both strict scenes while improving title exact coverage from `145` to `178` and reducing title sampled-object exact misses from `176` to `110`.
  - that flattened-selector ceiling is now resolved for the tracked title path: the current v18 package keeps the same strict title and file-select hashes as v14 while raising title exact coverage to `184` and dropping title sampled-object exact misses to `0`.
  - the active selector model is now richer but narrower, not broader: fully resolved surfaces compile as `upload-only`, while unresolved ordered surfaces compile as `dual`; for `surface-7701ac09`, the ordered path is now limited to the three unresolved edge cases instead of the full 53-slot strip.
  - practical implication: the first ordered-surface/runtime selector model is now doing useful work without destabilizing either tracked scene. File select stays byte-identical to the prior merged package, and title only consumes ordered fallback where the surface map actually has unresolved edge holes.
