...
- [`tools/hires_apply_surface_transport_policy.py`](/home/auro/code/parallel-n64/tools/hires_apply_surface_transport_policy.py)
  - applies tracked slot-alias policy onto a surface package before compile/build, so resolved edge behavior can be promoted from review artifacts into reproducible package inputs
- [`tools/hires_surface_transport_policy.json`](/home/auro/code/parallel-n64/tools/hires_surface_transport_policy.json)
...
- newer v26-context validation overturns that provisional selection for product use: removing `706` from the active merged package is materially closer to the legacy file-select image than `af028e08`, `81b32e31`, `c3984de7`, or `e3314872`, while strict title stays byte-identical
- practical implication: `7064585c` remains a useful review pool, but not an active imported selection, until a stronger selector model can beat the unresolved case in the current merged package context
- newer merged-package validation also overturns the old deterministic `c139c1c0` transport selection for product use: removing `c139` is materially closer to the legacy file-select image than either transported `640x160` payload, while strict title stays byte-identical
- practical implication: `c139c1c0` remains a valid sampled-object lookup proof, but not an active imported selection, until merged-package evidence can justify restoring one of its payloads
...
- The same bridge now also composes with the selected-package builder:
  - active combined package: [`20260329-selected-plus-title-v20-policy-aliased/package.phrb`](/home/auro/code/parallel-n64/artifacts/hires-pack-review/20260329-selected-plus-title-v20-policy-aliased/package.phrb)
  ...
  - self-contained surface-package proof:
    - alias-extension proof: reintroducing [`20260328-title-only-289/bindings.json`](/home/auro/code/parallel-n64/artifacts/hires-pack-review/20260328-title-only-289/bindings.json) on top of the active merged package produces [`20260329-selected-plus-title-v20-policy-aliased/package.phrb`](/home/auro/code/parallel-n64/artifacts/hires-pack-review/20260329-selected-plus-title-v20-policy-aliased/package.phrb), keeps title/file-select hashes unchanged, and raises title sampled-object exact hits from `145` to `178`
    - selector-shape finding: the original flattened compiled selector model hit a ceiling on title because it forced resolved and unresolved strip content through the same selector shape. The richer model now resolves that without widening matching: fully resolved surfaces compile as `upload-only`, unresolved surfaces compile as `dual`, and `surface-7701ac09` keeps only three ordered fallback selectors for its unresolved edge holes.
