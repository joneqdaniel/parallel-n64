...
- the first Tier 2 static scan is now live at [`tools/paper_mario_tier2_static_scan.py`](/home/auro/code/parallel-n64/tools/paper_mario_tier2_static_scan.py), with the latest comparison artifact at [20260328-title-copyright/report.md](/home/auro/code/parallel-n64/artifacts/paper-mario-tier2-static-scan/20260328-title-copyright/report.md):
  ...
  - the newer negative-space scan now rules out the easiest upstream false leads for that active strict `8x16` gap:
    - active file-select main-menu HUD scripts are `16x16`, `32x16`, and `64x16`, not a clean `8x16` match
    - active filemenu window-style texrect controls are `IA8 16x8` and `RGBA32 16x16`, not CI4
    - `draw_ci_image_with_clipping` remains a valid subrect-transport model, but its current upstream callers are message/sign/letter-style paths rather than an obvious strict file-select main-menu path
  - `kmr_21` now gives a direct title-screen source match for the `200x2 RGBA32` texrect stripes
  - `state_title_screen.c` now gives a direct source-backed match for the provisional `144x16` title pair: the non-JP copyright path draws two `144x16 IA8` chunks, and runtime exposes the same `144x16` sampled pair with matching recovered native tile fields (`sh=572`, `th=60`)
  - the separate `128x32` title seam from upload family `dfe97266` is now source-backed as well: the non-PAL `Press Start` path is a direct `128x32 IA8` loadblock with matching recovered native tile fields (`sh=508`, `th=124`)
  - practical implication: the remaining title IA8 work is no longer blocked on source attribution; it is blocked on transport/import coverage for the `Press Start` seam and broader package shaping for the larger title pools
