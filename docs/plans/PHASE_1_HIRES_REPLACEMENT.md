...
- Current meaning of that result:
  ...
  - the remaining title misses now look like a selector-shape ceiling in the flattened compiled handoff, not simple payload absence: `28916d63` and `940cea6e` each carry `29` transported candidates keyed by upload selectors, but the top probe misses arrive on collapsed canonical selectors (`0574791d28916d63` for `28916d63`, `00000000940cea6e` for `940cea6e`), which is evidence that the next general fix is a richer ordered-surface/runtime selector model rather than more ad hoc payload duplication
