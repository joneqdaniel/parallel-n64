...
- Current meaning of that result:
  ...
  - the old flattened-selector ceiling is now resolved on the tracked title path: the policy-built v20 package keeps the same strict title and file-select hashes as v14, but title sampled-object exact coverage rises to `184` and exact misses drop to `0`
