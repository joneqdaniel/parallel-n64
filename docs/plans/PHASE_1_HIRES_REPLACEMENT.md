          ...
          - the focused `469bad6f` sweep now tightens that further:
            ...
            - the new full native title package is now the best native title result so far:
              ...
              - title-family isolation now sharpens the next step:
                ...
                - the remaining low-complexity title one-offs are now `049201f4` and `ce437230`: the new title block-family probe [20260328-live-2-fullrow](/home/auro/code/parallel-n64/artifacts/paper-mario-title-block-family-probe/on/20260328-live-2-fullrow) proves their shared `2048x1 fs515` upload family `dfe97266` is not a truncated all-zero probe artifact but a real structured row with active bytes in `0x404..0xd77` and no current pack candidates
