...
- The exact sampled-object seam is now improved:
  ...
  - generic exact-checksum provider lookup now also returns the winning entry's native-versus-compat source class directly, so descriptor resolution and the upload path no longer have to re-probe the provider through a second checksum lookup just to distinguish native checksum fallback from compat fallback
  - the provider now also exposes a typed resolution contract for checksum-driven and upload-time lookup (`sampled-family-singleton`, `exact-native-sampled`, and generic native/compat classes), so the renderer's generic descriptor path and upload path no longer each open-code their own sampled-vs-native-vs-compat fallback ladders around raw provider primitives
  - the sampled-exact texrect / narrow triangle seam now also consumes a provider-owned sampled resolution contract (`exact-selector` versus `family-singleton`), so that path no longer rebuilds its first two sampled fallback stages from raw provider lookups before only the ordered-surface reservation step
  ...
  - when the upload path already knows a generic exact-checksum hit is compat-backed, it now materializes through the compat descriptor helper directly instead of re-entering the broad generic descriptor resolver just to decode the same resolved compat entry
  ...
  - draw-side exact sampled lookup no longer depends on sampled-object probe being enabled just to compute sampled identity; the new selected-package timeout conformance lane `emu.conformance.paper_mario_selected_package_timeout_lookup_without_probe` now proves the explicit `PHRB` timeout path stays hash-identical, `phrb-only`, and sampled-only with `PARALLEL_RDP_HIRES_SAMPLED_OBJECT_LOOKUP=1` and `PARALLEL_RDP_HIRES_SAMPLED_OBJECT_PROBE=0`, though the current probe-off residue is still the ordered-surface singleton half of the `940cea6e` texrect traffic (`descriptor_paths(sampled=33)` / `sampled_ordered_surface_singleton=33`) rather than the full probe-on `66`-hit contract
- The earliest safe conversion front door exists:
  ...
  - those review-only converter inputs now also participate in `--reuse-existing` validity through path+fingerprint request signatures, so changing a duplicate/alias/profile review invalidates stale canonical-package reuse instead of silently re-emitting the old shaping
  - those same review-only converter inputs are now explicitly optional overlays on the generic front door: when the current conversion scope does not include the reviewed family or only carries a candidate-free canonical sampled record, `hts2phrb` records the duplicate/alias input as `skipped` instead of failing the whole canonical-package run
  - the converter report now also classifies each review overlay as `none`, `applied`, `skipped`, or `mixed`, and the current Paper Mario timeout review-profile path now has a direct support contract that requires those overlays to remain explicit and reusable instead of silently disappearing or aborting conversion
  ...
  - the converter report now also lifts blocker runtime-state counts and review-backed blocker reasons to the top level, including an explicit uncovered blocker-family count when unresolved-review reasons do not explain every blocker; practical consequence: full-cache runs no longer require manual correlation between `promotion_blockers` and the unresolved-family review to understand what is actually still blocking promotion
  - the converter report now also lifts runtime-overlay reason counts and hash-review classes to the top level, so the remaining binding/transport residue is readable from `hts2phrb-report.json` without drilling into the nested overlay review first
  ...
  - there is now one tracked review-only reduction path for that overlay residue: [`tools/hires_runtime_overlay_review_profile.json`](/home/auro/code/parallel-n64/tools/hires_runtime_overlay_review_profile.json) plus [`tools/hires_runtime_overlay_review_transport_policy.json`](/home/auro/code/parallel-n64/tools/hires_runtime_overlay_review_transport_policy.json). The latest persistent proof at [`artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json) reduces the overlay from `15` bindings / `13` unresolved to `19` bindings / `9` unresolved, and the top-level split now makes that shape explicit as `8` direct overlay cases plus the same `1` import-linked case. It also eliminates the `candidate-set-equivalent` blocker class and the grouped candidate-set review artifact entirely (`runtime_overlay_candidate_set_review_group_count=0`), and still passes strict full-cache authority conformance. It remains review-only until promotion gates intentionally absorb it.
  ...
  - the latest full-cache drop is not heuristic drift: the converter now has one bounded offline promotion rule for `exact-family-ambiguous` residue, namely two-group exact-authoritative families whose groups are internally identical and exact nearest-neighbor scale-equivalents. That narrow rule collapses `7` Paper Mario families into deterministic exact-authority selections without adding any new runtime rule, which is why the current blocker counts fell from `379/375` to `372/368` on the current local cache and from `384/380` to `377/373` on the local pre-v401 cache
  ...
  - those generic detail counts now also split plain generic traffic into `native`, `compat`, and `unknown` buckets while preserving the older aggregate `plain` count, so checksum-shaped fallback residue is easier to classify without breaking existing evidence consumers
  - the renderer's resident-image cache now also splits generic descriptor entries by that resolved source class, so native-backed generic traffic and compat-backed generic traffic can no longer share one generic cache bucket just because the checksum/selector tuple matches
  - the final generic decode path now also uses the matching native or compat provider decode helper when that source class is known, so the remaining generic descriptor path no longer re-enters the broad generic decode helper just to materialize the image bytes
...
- Fresh authority reruns make the current runtime split explicit:
  - the repo-default title-screen, file-select, and `kmr_03 ENTRY_5` authority scenarios now prefer the current enriched full-cache `PHRB` artifact at [`artifacts/hts2phrb-review/20260408-pm64-all-families-authority-context-abs-summary/package.phrb`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260408-pm64-all-families-authority-context-abs-summary/package.phrb), then fall back only to the older enriched artifact and fail closed if neither promoted enriched runtime cache exists
  ...
  - practical consequence: the active authority lane has now intentionally moved onto the enriched full-cache `PHRB` runtime path; the zero-context compat-only `PHRB` artifact remains a front-door proof, but it is no longer part of the repo-default authority baseline and must now be exercised explicitly through `EMU_RUNTIME_PM64_FULL_CACHE_PHRB` or the zero-config refresh lane
