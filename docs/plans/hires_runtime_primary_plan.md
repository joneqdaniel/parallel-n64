...
- The earliest safe conversion front door exists:
  ...
  - there is now one tracked review-only reduction path for that overlay residue: [`tools/hires_runtime_overlay_review_profile.json`](/home/auro/code/parallel-n64/tools/hires_runtime_overlay_review_profile.json) plus [`tools/hires_runtime_overlay_review_transport_policy.json`](/home/auro/code/parallel-n64/tools/hires_runtime_overlay_review_transport_policy.json). The latest persistent proof at [`artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json) reduces the overlay from `15` bindings / `13` unresolved to `19` bindings / `9` unresolved, eliminates the `candidate-set-equivalent` blocker class and the grouped candidate-set review artifact entirely (`runtime_overlay_candidate_set_review_group_count=0`), and still passes strict full-cache authority conformance. It remains review-only until promotion gates intentionally absorb it.
