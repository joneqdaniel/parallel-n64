...
- The exact sampled-object seam is now improved:
  ...
  - ordered-surface singleton sampled families now resolve through the provider's sampled-family singleton helper instead of bespoke upload-path logic in the renderer, so the renderer no longer has to reimplement that one-family selection rule outside the provider
