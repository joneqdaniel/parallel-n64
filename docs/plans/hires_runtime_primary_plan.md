...
- The earliest safe conversion front door exists:
  ...
  - `--reuse-existing` now invalidates on runtime-class drift too, not just total runtime-ready counts, so stale manifests cannot silently reuse a report that claims the wrong native-vs-compat package shape
