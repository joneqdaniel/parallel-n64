...
trap 'rm -f "$log_file" "$shot_file"' EXIT
...
if [[ $rc -ne 0 ]]; then
  cat "$log_file"
  echo "FAIL: frame-hash run exited with status $rc." >&2
  exit 1
fi

if [[ ! -s "$shot_file" ]]; then
  cat "$log_file"
  echo "FAIL: screenshot file was not produced." >&2
  exit 1
fi

if ! grep -Eqi 'llvmpipe|lavapipe' "$log_file"; then
  cat "$log_file"
  echo "FAIL: frame-hash run did not report a software Vulkan device." >&2
  exit 1
fi

if ! grep -q 'plugin_start_gfx success\.' "$log_file"; then
  cat "$log_file"
  echo "FAIL: frame-hash run did not reach plugin_start_gfx success." >&2
  exit 1
fi

actual_sha256="$(sha256sum "$shot_file" | awk '{ print $1 }')"
if [[ "$actual_sha256" != "$EXPECTED_SHA256" ]]; then
  echo "FAIL: lavapipe frame hash mismatch." >&2
...
echo "emu_conformance_lavapipe_frame_hash: PASS ($actual_sha256)"
