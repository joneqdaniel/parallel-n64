...
# Boots SM64 with the PHRB pack for 30s, captures keying summary,
# asserts provider=on, entries > 0, compat_draw_hits > 0.
...
RUN_SECONDS=30
LOG_DIR="$(mktemp -d /tmp/sm64-hires-boot-XXXXXX)"
...
if entries < 1:
    errors.append(f"entries={entries}, expected > 0")
...
print(f"SM64 hi-res boot: provider={provider} entries={entries} compat_draw_hits={compat_draw_hits}")
PY
