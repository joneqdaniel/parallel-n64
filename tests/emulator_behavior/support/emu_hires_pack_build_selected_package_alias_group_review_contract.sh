...
if int(record.get("asset_candidate_count") or -1) != 54:
    raise SystemExit(f"FAIL: unexpected aliased asset_candidate_count {record.get('asset_candidate_count')!r}.")
...
if int(package_record.get("duplicate_pixel_group_count") or -1) != 1:
    raise SystemExit(
        f"FAIL: expected broader duplicate pixel group to remain after alias review, got {package_record!r}."
    )
...
print("emu_hires_pack_build_selected_package_alias_group_review_contract: PASS")
PY
