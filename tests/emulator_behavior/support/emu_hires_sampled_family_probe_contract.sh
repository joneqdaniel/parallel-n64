...
Hi-res keying summary: lookups=48467 hits=0 misses=48467 filtered=0 block_probe_hits=0 provider=on entries=66 native_sampled=65 compat=1 sampled_index=65 sampled_families=4 compat_low32_families=1 sources(phrb=65 hts=1 htc=0).
...
Hi-res sampled-object family: available=1 draw_class=texrect cycle=copy tile=0 sampled_low32=1b8530fb palette_crc=52e0d253 fs=258 selector=52e0d2531b8530fb prefer_exact_fs=1 exact_entries=33 generic_entries=0 active_entries=33 unique_checksums=33 unique_selectors=33 zero_selectors=0 matching_selectors=0 ordered_selectors=0 repl_dims=1 uniform_repl_dims=1 sample_repl=1184x24 active_is_pool=1 sample_policy=sampled-fmt2-siz1-off0-stride296-wh296x6-fs258-low321b8530fb-pool sampled_object=sampled-fmt2-siz1-off0-stride296-wh296x6-fs258-low321b8530fb.
...
check(summary.get("source_mode") == "mixed", f"unexpected summary source mode: {summary}")
...
check((summary.get("source_counts") or {}).get("phrb") == 65, f"unexpected phrb count: {summary}")
check((summary.get("source_counts") or {}).get("hts") == 1, f"unexpected hts count: {summary}")
check((summary.get("source_counts") or {}).get("htc") == 0, f"unexpected htc count: {summary}")
