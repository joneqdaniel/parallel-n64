...
def main():
    parser = argparse.ArgumentParser(description="Summarize hi-res pack low32 families for CI/import planning.")
    parser.add_argument("--cache", required=True, help="Path to .hts or .htc pack.")
    parser.add_argument("--bundle", help="Optional bundle path; if provided, analyze low32/fs pairs from ci_palette_probe.families.")
    parser.add_argument("--low32", action="append", default=[], help="Low32 texture CRC in hex, e.g. 42779bdd.")
    ...
    report = {
        ...
        "families": [
            build_family_summary(entries, texture_crc, formatsize)
            for texture_crc, formatsize in deduped_pairs
        ],
    }
