...
def build_map(sequence: dict, group: dict, sampled_low32: str):
    ...
    for row in sequence.get('sequences', []):
        key = row['key'].lower().replace('00000000', '') if len(row['key']) == 16 and row['key'].startswith('00000000') else row['key'].lower()
        candidate = by_key.get(key)
        item = {
            'sequence_index': row['sequence_index'],
            'addr_hex': row['addr_hex'],
            'upload_key': row['key'].lower(),
...
def render_markdown(result: dict):
    ...
    if result['unresolved_sequences']:
        for row in result['unresolved_sequences']:
            lines.append(f"- seq `{row['sequence_index']}` addr `{row['addr_hex']}` key `{row['upload_key']}`")
    else:
        lines.append('- none')
    lines.extend(['', '## Ordered Map', '', '| seq | addr | key | replacement | dims |', '|---:|---|---|---|---|'])
    for row in result['surface_map']:
        lines.append(
            f"| `{row['sequence_index']}` | `{row['addr_hex']}` | `{row['upload_key']}` | `{row.get('replacement_id', '-')}` | `{row.get('dims', '-')}` |"
        )
    lines.append('')
    return '\n'.join(lines)
...
def main():
    parser = argparse.ArgumentParser(description='Map ordered title upload sequences onto transported candidates.')
