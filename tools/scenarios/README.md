...
- [`paper-mario-title-screen.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-title-screen.sh), [`paper-mario-file-select.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-file-select.sh), and [`paper-mario-kmr-03-entry-5.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-kmr-03-entry-5.sh) now share the same default cache resolver for normal `on` authority runs: current enriched full-cache `PHRB`, then older enriched `PHRB`, then zero-config full-cache `PHRB`, and only then the legacy `.hts` cache.
