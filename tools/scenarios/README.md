...
- [`paper-mario-title-screen.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-title-screen.sh), [`paper-mario-file-select.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-file-select.sh), and [`paper-mario-kmr-03-entry-5.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-kmr-03-entry-5.sh) now share the same default cache resolver for normal `on` authority runs: current enriched full-cache `PHRB`, then older enriched `PHRB`, then zero-config full-cache `PHRB`, and then a hard failure if none of those promoted runtime artifacts exist.
...
- [`paper-mario-full-cache-phrb-authority-validation.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-full-cache-phrb-authority-validation.sh) is the full-cache `PHRB` authority runner: by default it exercises the zero-config `hts2phrb` output and proves that path is `provider=on` + `source_mode=phrb-only` even when the package is still compat-backed, and the current context-enriched authority-summary-root package now proves the stricter `phrb-only` + fully sampled authority lane as well
