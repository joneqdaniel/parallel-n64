...
- [`paper-mario-phrb-authority-validation.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-phrb-authority-validation.sh) is the shared Paper Mario `PHRB` authority runner. It executes title screen, file select, and `kmr_03 ENTRY_5` against a supplied `PHRB`, records one validation summary, and defaults the runtime source policy to `phrb-only`.
