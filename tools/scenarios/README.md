...
- [`paper-mario-full-cache-phrb-authority-refresh.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-full-cache-phrb-authority-refresh.sh) is the current best full-cache refresh workflow: it rebuilds the enriched authority-summary-root `PHRB` from the legacy Paper Mario `.hts` cache, gates the converter on `context_bundle_class=context-enriched` plus `package_manifest_runtime_ready_record_class=mixed-native-and-compat`, then runs the full-cache authority validation wrapper on that regenerated package so the latest converter/runtime state can be re-proved from one command
