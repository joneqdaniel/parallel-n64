...
- [`paper-mario-full-cache-phrb-authority-refresh.sh`](/home/auro/code/parallel-n64/tools/scenarios/paper-mario-full-cache-phrb-authority-refresh.sh) is the promoted full-cache baseline refresh workflow: it rebuilds the enriched authority-summary-root `PHRB` from the legacy Paper Mario `.hts` cache, gates the converter on `context_bundle_class=context-enriched` plus `package_manifest_runtime_ready_record_class=mixed-native-and-compat`, and proves the live promoted overlay shape too (`15` deterministic bindings, `13` unresolved overlay families on the promoted baseline artifact) together with the emitted `hts2phrb-runtime-overlay-review.*` artifact. That lane is the maintained default baseline, not the latest review-only reduction lane.
- [`tools/hires_runtime_overlay_review_profile.json`](/home/auro/code/parallel-n64/tools/hires_runtime_overlay_review_profile.json) is the tracked review-only reduction input for that same full-cache lane. It now combines the bounded `fs259` / `fs4` transport selections from [`tools/hires_runtime_overlay_review_transport_policy.json`](/home/auro/code/parallel-n64/tools/hires_runtime_overlay_review_transport_policy.json) with the proven sixty-six-family canonical selection set in [`tools/hires_canonical_family_selection_review.json`](/home/auro/code/parallel-n64/tools/hires_canonical_family_selection_review.json). The latest persistent proof at [`artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json) holds the overlay residue at `19` bindings / `9` unresolved with the top-level split reading `8` direct overlay cases plus `1` import-linked case, and lowers canonical-only residue from the promoted baseline's `368` families in `134` review groups to `302` families in `75` review groups while still passing the strict full-cache authority wrapper. The exact-scale same-aspect/context-bundle bucket is exhausted, the tiny same-aspect pairs are now largely mined out, and the remaining shallow residue is now dominated by larger same-aspect and clustered groups. The paired [`hts2phrb-runtime-overlay-linked-import-review.md`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-runtime-overlay-linked-import-review.md) keeps the only remaining non-deferred overlay blocker explicit as the `7064585c` linked-import cluster. It stays explicitly non-default.
