...
usage() {
  cat <<'USAGE'
Usage:
  tools/scenarios/paper-mario-full-cache-phrb-zero-config-refresh.sh [options]
...
if [[ -z "$OUTPUT_DIR" || -z "$BUNDLE_ROOT" ]]; then
  ...
  if [[ -z "$OUTPUT_DIR" ]]; then
    OUTPUT_DIR="$REPO_ROOT/artifacts/hts2phrb-review/${timestamp}-pm64-all-families-zero-config-refresh"
  fi
  if [[ -z "$BUNDLE_ROOT" ]]; then
    BUNDLE_ROOT="$REPO_ROOT/artifacts/paper-mario-probes/validation/${timestamp}-full-cache-phrb-authorities-zero-config-refresh"
  fi
fi
...
converter_args=(
  ...
  --expect-runtime-ready-class compat-only
...
if [[ ! -f "$PACKAGE_PATH" ]]; then
  echo "Missing refreshed zero-config package: $PACKAGE_PATH" >&2
  exit 1
fi
if [[ ! -f "$REPORT_PATH" ]]; then
  echo "Missing zero-config converter report: $REPORT_PATH" >&2
  exit 1
fi
...
if [[ ! -f "$SUMMARY_PATH" ]]; then
  echo "Missing zero-config authority validation summary: $SUMMARY_PATH" >&2
  exit 1
fi

echo "[zero-config-refresh] converter report: $REPORT_PATH"
echo "[zero-config-refresh] validation summary: $SUMMARY_PATH"
