...
PARALLEL_N64_GFX_PLUGIN_OVERRIDE="parallel" PARALLEL_RDP_HIRES_CACHE_PATH="$PACK_PATH" PARALLEL_RDP_HIRES_DEBUG="$([[ "$MODE" == "on" ]] && echo 1 || echo 0)" "$REPO_ROOT/tools/adapters/retroarch_stdin_session.sh"   --bundle-dir "$BUNDLE_DIR"   --mode "$MODE"   --retroarch-bin "$RETROARCH_BIN"   --base-config "$RETROARCH_BASE_CONFIG"   --core "$CORE_PATH"   --rom "$ROM_PATH"   --startup-wait "$STARTUP_WAIT"   --command "WAIT_COMMAND_READY 120"   --command "LOAD_STATE_SLOT_PAUSED 0"   --command "STEP_FRAME ${POST_LOAD_SETTLE_FRAMES}"   --command "WAIT_STATUS_FRAME PAUSED ${POST_LOAD_SETTLE_FRAMES} 10"   --command "SNAPSHOT_CORE_MEMORY paper-mario-gamestatus 800740aa 230"   --command "SNAPSHOT_CORE_MEMORY paper-mario-curgamemode 80151700 20"   --command "SNAPSHOT_CORE_MEMORY paper-mario-transition 800a0944 8"   --command "SNAPSHOT_CORE_MEMORY paper-mario-title-block-family-span ${PROBE_MIN_ADDR} ${PROBE_SPAN_BYTES}"   --command "SCREENSHOT"   --command "WAIT_NEW_CAPTURE 10"   --command "QUIT"
...
python3 "$REPO_ROOT/tools/hires_block_family_probe.py" analyze   --plan "$PLAN_JSON"   --snapshot-trace "$BUNDLE_DIR/traces/paper-mario-title-block-family-span.core-memory.txt"   --cache "$PACK_PATH"   --output-json "$REPORT_JSON"   --output-markdown "$REPORT_MD"
