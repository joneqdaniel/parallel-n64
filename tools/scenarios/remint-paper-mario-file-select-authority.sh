...
EXPECTED_SCREENSHOT_SHA256="f81d11bb9946ee49aec6691823fff03916f43e87c5f63f839a614573dd54c644"
...
"$REPO_ROOT/tools/adapters/retroarch_stdin_session.sh" \
  ...
  --command "WAIT_STATUS_FRAME PAUSED $(( POST_LOAD_SETTLE_FRAMES + FILE_SELECT_START_HOLD_FRAMES )) 10" \
  --command "CLEAR_INPUT_PORT 0" \
  --command "STEP_FRAME ${SAVE_OFFSET_FRAMES}" \
  --command "WAIT_STATUS_FRAME PAUSED $(( POST_LOAD_SETTLE_FRAMES + FILE_SELECT_START_HOLD_FRAMES + SAVE_OFFSET_FRAMES )) 10" \
  --command "SAVE_STATE" \
  --command "WAIT 1" \
  --command "QUIT"
